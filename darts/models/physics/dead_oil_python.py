...
class DeadOil:
    ...
    def __init__(self, timer, physics_filename, n_points, min_p, max_p, min_z):
        ...
        # water stream
        self.water_inj_stream = value_vector([0.999])
    ...
    def set_nonuniform_initial_conditions(self, mesh, nonuniform_pressure, nonuniform_composition):
        ...
        Arguments:
            -mesh: mesh object
            -nonuniform_pressure: nonuniform pressure setting
            -nonuniform_composition: nonuniform uniform_composition setting
        """
        assert isinstance(mesh, conn_mesh)
        nb = mesh.n_blocks
        ...
        composition = np.array(mesh.composition, copy=False)
        for c in range(self.n_components - 1):
            composition[c::(self.n_components - 1)] = nonuniform_composition[c]
