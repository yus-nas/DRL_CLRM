...
class GTrXLNet(RecurrentNetwork):
    ...
    def __init__(self,
        ...
        # Preprocess observation with CNN
        x = tf.keras.layers.Reshape((-1,4,23))(input_layer)
        x = tf.keras.layers.Conv1D(64, 2, activation='relu', input_shape=(4,23))(x)
        x = tf.keras.layers.Conv1D(64, 3, activation='relu')(x)
        x = tf.keras.layers.Reshape((-1,64))(x)
        
        # Map observation dim to input/output transformer (attention) dim.
        E_out = tf.keras.layers.Dense(self.attention_dim)(x)
